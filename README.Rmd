...
# Part 1: Building a Book

1. we could not compile the book, so no idea yet.

2. The biggest problem when we building the book is that many library are missing. To resolve it, we need to download it manually. Anothe problem is that 


# Part 2: Weather Stations
...
ggplot(data = states) + 
  geom_polygon(aes(x = long, y = lat, group = group), color = "grey",alpha = 0.6) + 
  coord_fixed(1.3) +
  guides(fill=FALSE)+
